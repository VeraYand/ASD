...
template <class TKey, class TVal>
int SortedTableOnArr<TKey, TVal>::lower_bound_index(const TKey& key) const {
    int left = 0;
...
template <class TKey, class TVal>
void SortedTableOnArr<TKey, TVal>::insert(const TKey& key, const TVal& value) {
...
template <class TKey, class TVal>
void SortedTableOnArr<TKey, TVal>::erase(const TKey& key) {
    int pos = lower_bound_index(key);
    if (pos < _rows.size() && _rows[pos].first == key) {
...
template <class TKey, class TVal>
bool SortedTableOnArr<TKey, TVal>::is_empty() const noexcept {
    return _rows.size() == 0;
}

template <class TKey, class TVal>
void SortedTableOnArr<TKey, TVal>::print(std::ostream& out) const {
    out << "{ ";
...
template <class TKey, class TVal>
void SortedTableOnArr<TKey, TVal>::read(std::istream& in) {
    TKey k;
    TVal v;
    if (in >> k >> v) {
        insert(k, v);
