...
namespace bench {
    ...
    // В одном эксперименте: 10 случайных операций; эксперимент повторяется 10 раз.
    inline constexpr size_t kRandomOperationCount = 10;
    inline constexpr size_t kRandomTrialCount = 10;

    inline size_t benchmark_trial_count(size_t /*n*/) {
    ...
    // C ≈ 4.5n: при равномерном хеше доля коллизий ~10–12% при n ключах.
        ...
        double grow_to(size_t n, const BenchmarkDataset& master, FillOrder order) {
            ...
            for (size_t idx : indices) {
                try {
                    ...
                    // Ключ уже в таблице после случайных вставок на предыдущем шаге.
                }
                ++inserted;
        ...
        int max_degree = 8)
    {
        ...
        for (size_t i = 0; i < num_terms; ++i) {
            ...
            int powers[3] = { degree_dist(gen), degree_dist(gen), degree_dist(gen) };
            ...
            for (int attempt = 0; attempt < 16 && !done; ++attempt) {
                ...
                try {
                    if (op == 0) { // Insert
                        ...
                        auto t0 = Clock::now();
                        volatile auto& sink = table.find(data.string_keys[key_index]);
                        (void)sink;
                        auto t1 = Clock::now();
